...
import 'package:bloom_app/widgets/monetization/bloom_inline_unlock_card.dart';
...
import 'bloom_results_constants.dart';
import 'bloom_results_strings.dart';
import 'widgets/bloom_results_streak_badge.dart';
import 'widgets/bloom_results_streak_row.dart';
...
class BloomResultsOkScreen extends StatefulWidget {
  final int streak;
  ...
  /// Whether the user has completed a Bloom session today (data-layer signal).
  final bool completedToday;

  const BloomResultsOkScreen({
    super.key,
  ...
  @override
  State<BloomResultsOkScreen> createState() => _BloomResultsOkScreenState();
}

class _BloomResultsOkScreenState extends State<BloomResultsOkScreen>
  ...
  void initState() {
    ...
    if (kDebugMode) {
      BloomDebugActions.instance.registerAction('Replay Animation', () {
        BloomLogger.instance.info('Debug: Replaying streak animation...');
        _triggerAnimation();
      });
    }
  ...
  void dispose() {
    if (kDebugMode) {
      BloomDebugActions.instance.unregisterAction('Replay Animation');
    }
        ...
        child: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.symmetric(
              horizontal: BloomResultsConstants.horizontalPadding,
            ),
            child: Column(
              ...
              children: [
                ...
                Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        BloomResultsStrings.dayOfStreak(displaySessionsNow),
                        style: textTheme.bodyMedium?.copyWith(
                      ...
                      const SizedBox(
                        height: BloomResultsConstants.verticalSpacingMedium,
                      ),
                      ...
                      AnimatedBuilder(
                        ...
                        builder: (context, _) {
                          ...
                          return BloomResultsStreakBadge(
                      ...
                      const SizedBox(
                        height: BloomResultsConstants.verticalSpacingSmall,
                      ),
  
                      // Small flame row
                      BloomResultsStreakRow(
                ...
                if (showPracticeUnlock) ...[
                  BloomInlineUnlockCard(
                    title: 'You’re building momentum.',
                    subtitle:
                        'Bloom+ Premium includes deeper practices, progress-based unlocks, and expanded affirmation libraries.',
                ...
                BloomPrimaryButton(
                  label: BloomResultsStrings.continueButton,
                    onPressed: () async {
                      ...
                      Navigator.of(context).pushReplacement(
                        MaterialPageRoute(
                          builder: (_) => BloomAffirmationUnlockedScreen(streak: totalSessions),
                        ),
                      );
                      return;
                    }
                    ...
                    Navigator.of(context).pushAndRemoveUntil(
                      MaterialPageRoute(
                        builder: (_) => const BloomShellScreen(),
                      ),
                      (route) => false,
                    );
                  },
...
class BloomAffirmationUnlockedScreen extends StatefulWidget {
  final int streak; // Renamed conceptually to total sessions in usage

  const BloomAffirmationUnlockedScreen({
    super.key,
    required this.streak,
  });

  @override
  State<BloomAffirmationUnlockedScreen> createState() =>
      _BloomAffirmationUnlockedScreenState();
}

class _BloomAffirmationUnlockedScreenState
    extends State<BloomAffirmationUnlockedScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      Future<void>.delayed(const Duration(milliseconds: 880), () {
        if (!mounted) return;
        SoundscapeService.instance.playSfx(BloomAssets.affirmationRevealSfx);
      });
