...
import '../../../data/streak/bloom_streak_service.dart';
...
/// Central brain for the Bloom Breath screen.
/// Manages: play/pause, wave phase, rising water, countdown timer.
class BloomBreathController extends ChangeNotifier {
  // Active breathing practice contract (default = Core Bloom)
  BreathingPracticeContract _contract = coreBloomContract;
  ...
  BloomBreathController({required TickerProvider vsync}) {
  ...
  Future<void> _handleSessionCompleteInternal() async {
    ...
    await BloomStreakService.recordSession(
      durationSeconds,
      practiceId: _contract.id,
    );

    // 4. Register streak completion (this calculates new streak)
    await BloomStreakService.registerSessionCompletedToday();
  }
