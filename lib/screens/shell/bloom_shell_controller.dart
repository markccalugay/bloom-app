...
class BloomShellController extends ChangeNotifier {
  int _currentIndex = 0;
  bool _isMenuOpen = false;
  int? _streak;
  String _displayName = 'Bloom guest';
  ...
  Future<void> loadStreak() async {
    try {
      _streak = await BloomStreakService.getCurrentStreak();
      notifyListeners();
  ...
  Future<void> _loadHomeHintState() async {
    ...
    if ((_streak ?? 0) >= 1 && !hasSeen) {
      _coachingStep = CoachingStep.bloomTimeButton;
    } else {
  ...
  Future<void> dismissHomeHint() async {
    if (_coachingStep == CoachingStep.bloomTimeButton) {
      HapticService.light();
